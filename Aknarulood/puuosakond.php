...
if (isset($_REQUEST["eiolevalmis"])) {
    global $yhendus;
    $kask = $yhendus->prepare("UPDATE tellimus SET puuvalmis = 0 WHERE id=?");
...
if (isset($_REQUEST["onvalmis"])) {
    global $yhendus;
    $kask = $yhendus->prepare("UPDATE tellimus SET puuvalmis = 1 WHERE id=?");
...
<div style="overflow-x: auto;">
    <table border="1">
        ...
        <?php
        global $yhendus;
        if (isset($_SESSION['kasutaja']) && $_SESSION['onAdmin'] == 2) {
            $kask = $yhendus->prepare("SELECT tellimus.id, tellimus_nimi, rulood.mustrinr, tellimus.riievalmis, tellimus.puuvalmis FROM tellimus INNER JOIN rulood ON tellimus.tellimus_nimi = rulood.id");
            $kask->bind_result($id, $tellimus_nimi, $ruloodmustrinr, $ruloodriievalmis, $tellimuspuuvalmis);
            $kask->execute();
            while ($kask->fetch()) {
                $tekst = "Valmis";
                $seisund = "onvalmis";
                $tekst2 = "Toode ei ole valmis";
                if ($tellimuspuuvalmis == 1) {
                    $tekst = "Ei ole valmis";
                    $seisund = "eiolevalmis";
